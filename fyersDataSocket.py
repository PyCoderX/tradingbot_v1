...
class DataSocket:
    def __init__(
        self, access_token, symbols, data_queue, filename: str, log_path: str | Path = ""
    ):
    ...
    def onmessage(self, message):
        ...
        if message.get("symbol"):
            df = pd.DataFrame.from_dict(message, orient="index").T
            df.to_csv(self.filename, index=None, sep=" ", mode="a")
